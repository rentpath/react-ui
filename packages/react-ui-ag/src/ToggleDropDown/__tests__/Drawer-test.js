...
import ThemedToggleDropDown from '../ToggleDropDown'
import theme from './mocks/theme'

const ToggleDropDown = ThemedToggleDropDown.WrappedComponent

describe('ToggleDropDown', () => {
  it('renders the base class and children', () => {
    const props = {
      theme,
      className: theme.ToggleDropDown,
      children: (<div>Content</div>),
    }

    const wrapper = mount(<ToggleDropDown {...props} />)
    expect(wrapper.hasClass('ToggleDropDown')).toBeTruthy()
    expect(wrapper.find('[data-tid="toggleDropDown-content"]').contains([<div>Content</div>])).toBeTruthy()
  })
  ...
  it('It fires callback function which returns alternating true and false', () => {
    ...
    const wrapper = mount(<ToggleDropDown onClick={onClick} {...props} />)
  ...
  it('hides content when visible is set to false', () => {
    ...
    const wrapper = mount(<ToggleDropDown {...props} />)
    expect(wrapper.find('[data-tid="ttoggleDropDown-content"]')).toHaveLength(0)
  })
})
